...
function SplashCursor({
  ...
  DENSITY_DISSIPATION = 2.8,  // 增加密度消散，让颜色消失更快
  VELOCITY_DISSIPATION = 1.5, // 增加速度消散，让运动停止更快
  PRESSURE = 0.8,
  PRESSURE_ITERATIONS = 20,
  CURL = 25,              // 稍微减少涡流，让效果更快平静
  SPLAT_RADIUS = 0.2,     // 减小爆炸半径
  SPLAT_FORCE = 4500,     // 减小爆炸强度
  SHADING = true,
  COLOR_UPDATE_SPEED = 10,
  BACK_COLOR = { r: 0, g: 0, b: 0 },
  TRANSPARENT = true
}) {
  console.log('🎨 SplashCursor 组件开始渲染');
  ...
  useEffect(() => {
    ...
    // 添加一个测试效果确认组件工作
    setTimeout(() => {
      console.log('🎯 测试自动动画效果');
      const pointer = pointers[0];
      updatePointerDownData(pointer, -1, canvas.width / 2, canvas.height / 2);
      clickSplat(pointer);
    }, 1000);
